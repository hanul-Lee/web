const projects = {
  intranet: {
    no:'04', type:'WORKFLOW MOBILE · B2B', title:'모바일 업무 포털과 예약 플로우', domain:'한맥기술 모바일 인트라넷', lead:'PC에서만 가능했던 결재와 예약 업무를 이동 중에도 바로 완료할 수 있도록, 모바일 중심의 업무 흐름으로 재설계했습니다.',
  ...
  dashboard: {
    no:'01', type:'DATA PRODUCT · B2B SAAS', title:'실시간 운영 의사결정 대시보드', domain:'원가·공수 분석 프로젝트', lead:'숫자는 매일 쌓였지만 결정은 늦었습니다. 흩어진 운영 데이터를 역할에 맞춘 실시간 판단 화면으로 바꿨습니다.',
  ...
  platform: {
    no:'02', type:'B2B PLATFORM · INFORMATION ARCHITECTURE', title:'멀티 유저 B2B 플랫폼 정보구조 재설계', domain:'건설 통합 플랫폼 프로젝트', lead:'서로 다른 목적을 가진 사용자들이 하나의 시스템 안에서 빠르게 탐색하고 판단할 수 있도록 정보구조와 핵심 흐름을 재설계했습니다.',
    ...
    outcomes:[['5.4 → 2','평균 탐색 깊이'],['25% 감소','이탈률'],['30% 향상','전환율']], images:8, next:['pet','반복 행동을 만드는 습관 형성 제품']
  },
  tbm: {
    no:'05', type:'FIELD WORKFLOW · MOBILE UX', title:'현장 기록의 모바일 전환 UX', domain:'TBM 안전 기록 시스템', lead:'종이와 PC에 묶여 있던 반복 기록을 야외에서도 바로 확인하고 완료할 수 있는 현장 중심 모바일 경험으로 전환했습니다.',
  ...
  pet: {
    no:'03', type:'CONSUMER PRODUCT · HABIT UX', title:'반복 행동을 만드는 습관 형성 제품', domain:'댕냥실록 · 개인 제품 프로젝트', lead:'기록해야 한다는 부담을 빠른 행동과 감성적 보상으로 바꿔, 건강 관리가 자연스럽게 반복되는 소비자 제품을 설계했습니다.',
    ...
    outcomes:[['62.4%','기존 앱 사용 이탈'],['45.8초','평균 기록 소요'],['2단계','핵심 기록 플로우']], images:8, next:['intranet','모바일 업무 포털과 예약 플로우']
  }
};
...
const renderStudy = (study) => `
  <article class="story-section evidence">
    <header><p>문제 발견</p><h3>${study.insight}</h3></header>
  ...
  <article class="story-section strategy">
    <header><p>설계 방향</p><h3>${study.strategyHeadline}</h3></header>
  ...
  <article class="story-section solution">
    <div class="solution-copy"><p>핵심 화면</p><h3>${study.uiTitle}</h3><span>${study.uiText}</span></div>
...
const renderImpact = (impact) => `
  <article class="story-section impact impact-${impact.type}">
    <header><p>${impact.label}</p><h3>${impact.title}</h3></header>
