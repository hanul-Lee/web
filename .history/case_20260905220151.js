const projects = {
  ...
  kanvan: {
    no:'09', type:'B2B SAAS · WORKFLOW UX · PROTOTYPE', title:'업무의 흐름이 보이는 칸반보드 서비스', domain:'Kanban Workflow Management', lead:'리스트 중심 업무관리에서 파악하기 어려웠던 상태와 우선순위를 카드와 컬럼으로 시각화하고, 핵심 업무 흐름을 검증할 수 있는 프로토타입으로 설계했습니다.',
    period:'2026.03 — 진행중', role:'문제 정의 · UX 구조 설계 · UI 디자인 · 프로토타이핑', contribution:'UX/UI 100% · 구현 보조', team:'개인 프로젝트 · AI 협업 프로토타이핑', bg:'#15162d', ink:'#fff', accent:'#ff8500', textAccent:'#d96800', muted:'#d4d5e2',
    problem:'제목과 등록일 중심의 목록만으로는 업무의 현재 상태와 다음 행동을 빠르게 판단하기 어려웠습니다.', detail:'진행·대기·완료 업무를 컬럼으로 구분하고, 카드 안에서 담당자와 우선순위를 바로 확인하도록 정보 위계를 재설계했습니다. 드래그앤드롭과 상세 모달은 AI의 도움을 받아 프로토타이핑했으며, 현재는 핵심 흐름과 예외를 다듬는 단계입니다.',
    outcomes:[['핵심 흐름','상태·우선순위·담당자 구조화'],['프로토타입','드래그앤드롭·상세 모달'],['검증 준비','다음 사용자 테스트 과제 정의']], outcomeBasis:'개인 프로젝트의 UX 설계와 AI 협업 프로토타이핑 결과 · 정식 사용자 테스트 전', next:['skyautonet','자율주행 기술을 이해시키는 웹 경험']
  },
  skyautonet: {
...
const caseStudies = {
  ...
  kanvan: {
    ...
    context:[['사용자','기획 · 디자인 · 개발 실무자'],['환경','데스크톱 기반 협업 도구'],['현재 단계','핵심 흐름 프로토타입 · 검증 준비']],
    ...
    visuals:[['assets/pp/kanvan/kanvan02.jpg','문제 정의','리스트형 게시판의 상태·우선순위·협업 정보 문제'],['assets/pp/kanvan/kanvan03.jpg','해결 과정','문제 정의부터 UX/UI 설계와 프로토타이핑까지의 과정'],['assets/pp/kanvan/kanvan04.jpg','Before & After','목록 탐색에서 직접 상태를 변경하는 구조로 전환'],['assets/pp/kanvan/kanvan05.jpg','프로토타입과 회고','드래그앤드롭과 협업 기능을 구현하며 확인한 후속 검증 과제'],['assets/pp/kanvan/kanvan06.jpg','다음 검증 과제','탐색 시간과 과제 성공률을 측정하기 위한 사용자 테스트 계획']],
    impact:{type:'workflow', label:'프로토타입 검증 범위', title:'리스트에서 업무 상태와 다음 행동이 보이는 보드로 전환했습니다.', before:'리스트 중심', after:'칸반 프로토타입', note:'상태·우선순위·담당자 탐색 흐름을 설계하고, 정식 사용자 테스트를 위한 과업을 정의했습니다.'},
    reflection:{learned:'모든 기능을 완성했다고 말하기보다, 어떤 업무 흐름을 왜 바꿨는지 설명하고 실제 검증 과제를 남기는 것이 중요하다는 점을 배웠습니다. 구현은 AI의 도움을 받아 진행했으며, 상태·예외·데이터 구조를 직접 확인하며 수정했습니다.', next:'다음 단계에서는 실무자 대상 과업 테스트로 탐색 시간과 오클릭을 측정하고, 실제 협업 환경에서 드래그앤드롭·검색·필터·알림의 우선순위를 검증하겠습니다.'}
  },
  skyautonet: {
...
if (p) {
  ...
  document.querySelector('.case-eyebrow').textContent=`PROJECT ${p.no} · ${p.type}`;
